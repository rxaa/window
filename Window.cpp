...
void sdf::Control::onMeasure() {
    ...
    if (hp != showX_ || vp != showY_ || pos.w != showW_ || pos.h != showH_)
        setPosAndHW(hp, vp, pos.w, pos.h);
...
void ScrollView::onMouseMove(int32_t x, int32_t y) {
    ...
    int32_t vX = x + getHoriPos();
    int32_t vy = y + getVertPos();
...
void ScrollView::onRightClick() {
    if (hoverView)
        hoverView->onRightClick();
}
