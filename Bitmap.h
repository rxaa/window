...
namespace sdf
{
	...
	public:
		...
		inline BOOL DrawStretchTo(Hdc toGdi, int toX, int toY, int toW, int toH, int fromW, int fromH, int fromX = 0, int fromY = 0)
		{
		...
		inline BOOL DrawAlphaTo(Hdc toGdi, int toX, int toY, int toW, int toH)
		{
			...
			return ::AlphaBlend(toGdi.GetDc(), toX, toY, toW, toH
				, hdc_, 0, 0, width_, height_
				, blendFunction);

		}

		inline BOOL DrawAlphaTo(Hdc toGdi, int toX, int toY, int toW, int toH, int fromW, int fromH, int fromX = 0, int fromY = 0)
		{
			BLENDFUNCTION  blendFunction;
			blendFunction.BlendFlags = 0; //没用
			blendFunction.AlphaFormat = AC_SRC_ALPHA; //AC_SRC_ALPHA表示图像中包含ALPHA值
